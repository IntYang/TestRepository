...
public class Exam07JspController {
	...
	public String exam05(Model model){// 매개값 Model: 데이터를 jsp에서 전달하고 싶다는 의미
			model.addAttribute("name3", "홍길동"); // add 여러개 추가할 수 잇고, set은 하나만 저장한다는의미로 해석하면 거의 맞음, add추가, set 갱신
			model.addAttribute("member3", new Member("홍길동", 30));
			
			Board board = new Board();
			board.setBno(1);
			board.setBtitle("오늘은 휴가 전날, 자바 학습");
